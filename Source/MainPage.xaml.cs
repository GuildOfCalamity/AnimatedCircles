...
public sealed partial class MainPage : Page
{
    ...
    {
        sldrFrequency.Value = 1;
        sldrDuration.Value = 7;
        if (_saveToDisk)
        {
            // Create a timer for capturing animation.
    ...
    async void _timer_Tick(object? sender, object e) => await UpdateScreenshot(animGrid, null);

    void SliderFrequencyChanged(object sender, Microsoft.UI.Xaml.Controls.Primitives.RangeBaseValueChangedEventArgs e)
    {
        if (_loaded)
        {   // We'll change by 1/10th with each tick.
            circles.AnimationFrequency = e.NewValue / 10d;
            Debug.WriteLine($"[INFO] AnimationFrequency is now {circles.AnimationFrequency}");
        }
    }

    void SliderDurationChanged(object sender, Microsoft.UI.Xaml.Controls.Primitives.RangeBaseValueChangedEventArgs e)
    {
        if (_loaded)
        {   // We'll change by 1/10th with each tick.
            circles.AnimationDuration = e.NewValue / 10d;
            Debug.WriteLine($"[INFO] AnimationDuration is now {circles.AnimationDuration}");
        }
    }
    ...
    void Button_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            _timer?.Start();
            circles.IsRunning = true; 
            btnRun.IsEnabled = sldrFrequency.IsEnabled = sldrDuration.IsEnabled = !circles.IsRunning;
            await Task.Delay(4000);
            circles.IsRunning = false;
            btnRun.IsEnabled = sldrFrequency.IsEnabled = sldrDuration.IsEnabled = !circles.IsRunning;
    ...
    async Task SaveImageSourceToFileAsync(Microsoft.UI.Xaml.Controls.Grid hostGrid, Microsoft.UI.Xaml.Media.ImageSource imageSource, string filePath, int width = 32, int height = 32)
    {
        ...
        // This is super clunky, but for some reason the Image resource is never
